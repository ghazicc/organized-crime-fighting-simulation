...
void monitor_gang_activity(PoliceOfficer* officer) {
    ...
    // Check if gang exists and has members
            ...
            for (int j = 0; j < officer->num_agents; j++) {
                if (officer->secret_agent_ids[j] == gang->members[i].agent_id) {
                    ...
                    printf("Police Officer %d: Receiving intel from Agent %d in Gang %d\n", 
                           officer->police_id, gang->members[i].agent_id, officer->gang_id_monitoring);
                    fflush(stdout);
                    break;
                }
            }
...
void arrest_gang_members(PoliceOfficer* officer) {
    ...
    for (int i = 0; i < gang->max_member_count; i++) {
        if (gang->members[i].is_alive && gang->members[i].agent_id < 0) { // Don't arrest agents
            if ((rand() % 100) / 100.0f < arrest_success_rate) {
                gang->members[i].is_alive = false;
    ...
    if (arrests_made > 0) {
        police_force.total_arrests += arrests_made;
